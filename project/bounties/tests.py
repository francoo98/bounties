...
class BountyDeleteViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        Bounty.objects.create(title="Test bounty", reward=300,
                              description="something", creator=user)
...
class BountyCreateTestCase(TestCase):
    ...
    def test_bounty_reward_constraint(self):
        user = User.objects.get(username="franco")
        self.assertRaises(django.db.utils.IntegrityError, Bounty.objects.create,
                          title="Test bounty", reward=-300, description="something", creator=user)

    def test_user_not_null(self):
        self.assertRaises(django.db.utils.IntegrityError, Bounty.objects.create,
                          title="Test bounty", reward=-300, description="something")
